...
const ExistingRooms = () => {
  ...
  const handleDelete = async (roomId) => {
    try {
      ...
      if (result === "") {
        setSuccessMassage(`Room No ${roomId} was deleted`);
        fetchRooms();
      } else {
        console.error(`Error deleting room : ${result.massage}`);
      }
    } catch (error) {
      setErrorMassage(error.massage);
    }
        ...
        <>
          <section className="mt-5 mb-5 container">
            ...
            <Row>
              ...
              <Col md={6} className="f-flex justify-content-end">
                <Link to={"/add-room"} className="btn btn-info sm">
                  <FaPlus />
                  Add Room
                </Link>
              </Col>
